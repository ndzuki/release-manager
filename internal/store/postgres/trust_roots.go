...
// GetActiveByEnvironment returns roots in active or grace state that accept
...
// The UPDATE on trust_policies locks the environment's policy row, so two
// concurrent transitions serialize even under READ COMMITTED: the second
// transaction blocks until the first commits and then sees its outcome.
func (s *trustRootStore) TransitionLiveRoot(ctx context.Context, id, env string, to store.TrustRootState, revokedAt *time.Time, bumpRevocation bool) (*store.TrustPolicyMeta, error) {
	...
	now := time.Now().UTC()
	// Environment write barrier: ensure the policy row exists, then lock it so
	// concurrent transitions for the same environment serialize.
	_, err = tx.ExecContext(ctx, `
INSERT INTO trust_policies (environment, version, revocation_epoch, updated_at)
VALUES (?, 0, 0, ?)
ON CONFLICT (environment) DO NOTHING
