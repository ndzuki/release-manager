...
| `make quality` | `sdk-check` + `test-coverage` + `lint` + `check-reqs` + `check-error-codes` + `check-licenses` + `check-docs` + `check-config-keys` + `check-migrations` + `check-probes` + `lint-proto` 的聚合门禁 | 同各子项 | 本地；CI 不直接调用，而是分 job 跑等价命令 |
