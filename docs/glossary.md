...
| env-config 组装 | REQ-066 Makefile target（e2e-stage/e2e-all/e2e-cleanup）从 REQ-065 既有输出组装 `--env-config`：local 可消费 `dev-credentials.env`，ci 消费已注入的 Secret env；`dev-status.json` / `dev-fixture.json` / `kubeconfig.yaml` 等非凭据输入仍按 profile 产出。REQ-065 不产运行时 YAML（D-023 D1、D-031）。schema 见 `contracts/e2e-environment-config.md`。 | 设计词汇表：REQ-066, D-023, D-031 |
