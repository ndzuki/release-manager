...
type auditEventStore struct{ gorm *DB }
...
func (s *auditEventStore) GetByID(ctx context.Context, id string) (*store.AuditEvent, error) {
	row := s.gorm.QueryRowContext(ctx, `SELECT `+auditEventColumns+` FROM audit_events WHERE id = ?`, id)
...
func (s *auditEventStore) Count(ctx context.Context, filter store.AuditEventFilter) (int64, error) {
	...
	var count int64
	if err := s.gorm.QueryRowContext(ctx, query, args...).Scan(&count); err != nil {
