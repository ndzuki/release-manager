...
func (s *userStore) Update(ctx context.Context, u *store.User) error {
	...
	if u.Status == store.UserDisabled {
		if _, err := tx.ExecContext(ctx, `
			UPDATE auth_sessions SET revoked = TRUE WHERE user_id = ?`, u.ID); err != nil {
...
// Count returns the number of users, optionally filtered by organization membership.
func (s *userStore) Count(ctx context.Context, orgID string) (int64, error) {
	var count int64
	var err error
	if orgID == "" {
		err = s.gorm.QueryRowContext(ctx, `SELECT COUNT(*) FROM users`).Scan(&count)
	} else {
		err = s.gorm.QueryRowContext(ctx, `SELECT COUNT(*) FROM users WHERE id IN (SELECT user_id FROM organization_members WHERE organization_id = ?)`, orgID).Scan(&count)
	}
	return count, err
}
...
func (s *userStore) List(ctx context.Context, query store.UserListQuery) (*store.UserPage, error) {
	...
	rows, err := s.gorm.QueryContext(ctx, querySQL, args...)
	if err != nil {
