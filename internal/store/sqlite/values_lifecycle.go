...
type valuesLifecycleStore struct{ db *sql.DB }
...
func (s *valuesLifecycleStore) CreateDraft(ctx context.Context, command store.CreateValuesDraftCommand) (*store.CreateValuesDraftResult, error) {
	return retryValuesLifecycle(ctx, command.IdempotencyKeyHash,
		func() (*store.CreateValuesDraftResult, error) { return s.createDraft(ctx, command) },
		func() (*store.CreateValuesDraftResult, error) { return s.loadCreateValuesReplay(ctx, command) },
	)
}
...
func (s *valuesLifecycleStore) createDraft(ctx context.Context, command store.CreateValuesDraftCommand) (*store.CreateValuesDraftResult, error) {
	if command.Revision == nil {
		return nil, fmt.Errorf("create values draft: nil revision")
	}
	...
	if err := tx.QueryRowContext(ctx, `SELECT id FROM release_definitions WHERE id = ?`, command.Revision.ReleaseDefinitionID).Scan(&definitionID); err != nil {
...
func (s *valuesLifecycleStore) Discard(ctx context.Context, command store.DiscardValuesCommand) (*store.DiscardValuesResult, error) {
	return retryValuesLifecycle(ctx, command.IdempotencyKeyHash,
		func() (*store.DiscardValuesResult, error) { return s.discard(ctx, command) },
		func() (*store.DiscardValuesResult, error) { return s.loadDiscardValuesReplay(ctx, command) },
	)
}
...
func (s *valuesLifecycleStore) discard(ctx context.Context, command store.DiscardValuesCommand) (*store.DiscardValuesResult, error) {
	...
	`, encodedNow, encodedNow, command.RevisionID, command.ExpectedStateVersion, command.ActorUserID)
	if err != nil {
	...
	if rows != 1 {
		if _, getErr := getValues(ctx, tx, command.RevisionID); getErr != nil {
			return nil, getErr
		}
...
func consumePrepareSession(ctx context.Context, tx *sql.Tx, command store.CreateValuesDraftCommand) (*store.PrepareSession, error) {
	now := time.Now().UTC()
	encodedNow := now.Format(time.RFC3339Nano)
	updated, err := tx.ExecContext(ctx, `
	...
	`, encodedNow, command.PrepareTokenHash, command.ActorUserID, command.OrganizationID,
		command.Revision.ReleaseDefinitionID, encodedNow)
	if err != nil {
...
func bindPrepareTasks(ctx context.Context, tx *sql.Tx, session *store.PrepareSession, revision *store.ValuesRevision) error {
	...
	rows, err := tx.QueryContext(ctx, convergenceTaskSelect+` WHERE release_definition_id = ?`, revision.ReleaseDefinitionID)
	if err != nil {
	...
	selected := make([]*store.ConvergenceTask, 0, len(session.TaskIDs))
	...
	paths, err := store.LockedPathsForTasks(session.TaskIDs, selected)
	if err != nil {
		return err
	}
...
func insertValuesDraft(ctx context.Context, tx *sql.Tx, revision *store.ValuesRevision) error {
...
func insertValuesCreatedEvent(ctx context.Context, tx *sql.Tx, command store.CreateValuesDraftCommand) error {
	...
	return insertApprovalOutbox(ctx, tx, "audit_outbox", &store.ApprovalOutboxEntry{
		ID: uuid.NewString(), EventType: "ValuesRevisionCreated", PayloadJSON: payload, CreatedAt: command.Revision.CreatedAt,
	})
}

func insertValuesDiscardedEvent(ctx context.Context, tx *sql.Tx, command store.DiscardValuesCommand, revision *store.ValuesRevision, now time.Time) error {
...
func lookupCreateValuesReplay(ctx context.Context, queryer operationQueryer, command store.CreateValuesDraftCommand) (*store.CreateValuesDraftResult, error) {
	...
	revision, err := getValues(ctx, queryer, ref.RevisionID)
	if err != nil {
		return nil, err
	}
...
func lookupValuesLifecycleReplay(ctx context.Context, queryer operationQueryer, scope, keyHash, requestHash string) (*valuesLifecycleReplayRef, error) {
	...
	`, scope, keyHash, time.Now().UTC().Format(time.RFC3339Nano)).Scan(&persistedHash, &responseRef)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
	if err != nil {
...
func insertCreateValuesIdempotency(ctx context.Context, tx *sql.Tx, command store.CreateValuesDraftCommand) error {
...
func insertDiscardValuesIdempotency(ctx context.Context, tx *sql.Tx, command store.DiscardValuesCommand, result *store.DiscardValuesResult) error {
	return insertValuesLifecycleIdempotency(ctx, tx, command.IdempotencyScope, command.IdempotencyKeyHash,
...
func insertValuesLifecycleIdempotency(ctx context.Context, tx *sql.Tx, scope, keyHash, requestHash string, expiresAt time.Time, ref valuesLifecycleReplayRef) error {
	if keyHash == "" {
		return nil
	}
	if expiresAt.IsZero() {
	...
	if _, err := tx.ExecContext(ctx, `DELETE FROM idempotency_records WHERE scope = ? AND text_key = ? AND expires_at <= ?`, scope, keyHash, now); err != nil {
	...
	if _, err := tx.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?)
	`, scope, keyHash, requestHash, encoded, expiresAt.UTC().Format(time.RFC3339Nano)); err != nil {
		if isUniqueConstraint(err) {
...
func (s *valuesLifecycleStore) loadCreateValuesReplay(ctx context.Context, command store.CreateValuesDraftCommand) (*store.CreateValuesDraftResult, error) {
	return lookupCreateValuesReplay(ctx, s.db, command)
}
...
func (s *valuesLifecycleStore) loadDiscardValuesReplay(ctx context.Context, command store.DiscardValuesCommand) (*store.DiscardValuesResult, error) {
	return lookupDiscardValuesReplay(ctx, s.db, command)
}

func retryValuesLifecycle[T any](
	ctx context.Context,
	idempotencyKeyHash string,
	action func() (*T, error),
	loadReplay func() (*T, error),
) (*T, error) {
	var result *T
	...
	if errors.Is(err, store.ErrIdempotencyConflict) && idempotencyKeyHash != "" {
		if replay, replayErr := loadReplay(); replayErr != nil || replay != nil {
