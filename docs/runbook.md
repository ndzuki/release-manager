...
- 超时常量（服务端唯一权威，写死在构造函数）：`sessionTTL = 15 * time.Minute`、`heartbeatMaxAge = 30 * time.Second`、`suspectAfter = 60 * time.Second`（`internal/operator/service.go:67-69`）。`SessionEstablished` 向 agent 广播 `HeartbeatIntervalSeconds = 15`、`HeartbeatTimeoutSeconds = 60`（`internal/operator/service.go:519-529`），Establish 路径写的 `ExpiresAt = now + 15m`（`internal/operator/service.go:443-453`）。这些值**不来自配置**。
...
- 重连与恢复（现状）：
  ...
  - 重连后第一个动作是 `handleReconnect`：取 `GetNextSequence()`（全局 outbox 序列）与 agent 上报的 `LastSeenSequence` 比对，**有 gap 只发 `ResyncRequest` 并等待，不立即重投**（`internal/operator/service.go:1104-1143`）；无 gap 才重投 `delivered-not-acked`（`internal/operator/service.go:1147-1187`），已完成的下发 `DuplicateResponse`。
  - 由于序列是全局的（`internal/operator/service.go:1111`），任一 operator 收到过命令都会让别的 operator 判定为 gap ⇒ 「sequence gap detected」日志（`internal/operator/service.go:1123-1126`）**在当前拓扑下是常态噪声**，不是故障证据。真正投递靠 5s 一轮的 `deliverPending`（`internal/operator/service.go:1264-1300`，受 `max_inflight=1` 约束）。
...
3. `renew_too_early`：不是故障，等到剩余有效期低于阈值再续（`internal/operator/service.go:1350`）。
...
- 签名密钥来源：`--signing-key`，默认值取环境变量 `JWT_SIGNING_KEY`，再默认 `change-me-in-production`（`cmd/auth/main.go:237`、`cmd/orchestrator/main.go:830-836`）。集群里由 Secret `release-manager-jwt` 注入（`deploy/kustomize/services/orchestrator.yaml:34-39`、`deploy/kustomize/services/auth.yaml:35-39`），Secret 内容由 `data/dev-jwt/jwt-signing-key.pem` 生成（`deploy/kustomize/dev/kustomization.yaml:39-43`）。
...
5. `invalid service token`（bundle 上传链路）：orchestrator 同时接受 current + previous 两个 token（`DEV_WEBHOOK_SERVICE_TOKEN` / `..._PREVIOUS`，`cmd/orchestrator/main.go:800-819`；Secret 键见 `deploy/kustomize/services/orchestrator.yaml:45-55`），发送端只用 current（`deploy/kustomize/services/webhook.yaml:38-43`）。⇒ **轮换必须两键并存**，只换一键会造成 ingress 全 403。
...
- 一次性恢复只在启动时跑（非维护模式）：`RecoverNonTerminal`（`cmd/orchestrator/main.go:335-340`）。其行为（`internal/orchestrator/operation/recover.go:14-30,58-87`）：
...
**处置后验证**：`ListStuckLocks` 不再包含该 intent；60s 扫描不再打 `emergency target lock is stuck`（`cmd/orchestrator/main.go:786-798`）；`QueryAuditEvents` 能看到 `emergency_lock_release`（注意 §10 的可见性缺口）。
...
11. **未接线组件**（现状 = 代码存在但生产不启用，排查时不要以它们为依据）：`internal/operator/session_registry.go:27-104`（在线性评估器：构造 + `Run` + `evaluate` 全无生产调用者）、`cmd/operator/main.go:303-336` 的 `runSessionExpiry`（agent 模式下 `s.st == nil` 直接 continue）、`internal/operator/session_client.go`（带心跳的会话客户端，无 `cmd/` 引用）、`internal/store/postgres/operators.go:787` 的 `UpdateStatusReason` 与 `internal/store/sqlite/operators.go:897` 同名方法（仅测试调用）。
12. **需核实（无法从代码判定）**：① 「全局 outbox 序列导致跨 operator 误判 gap」是否为有意设计（`internal/operator/service.go:1111`）；② `agent.mode` 何时切到 `gateway`（`cmd/operator/main.go:28-38` 注释称 TASK-065 将移除该路径）；③ 生产形态（本仓只有 `deploy/kustomize/dev` 一套 overlay）下的 retention/告警责任方。
