		...
		default:
			...
			case req.GetResult() != nil:
				...
				if err := s.store.Outbox().UpdateStatus(ctx, result.GetOutboxId(), status, resultJSON); err != nil {
					return fmt.Errorf("persist legacy command result outbox: %w", err)
				}
				if existing != nil {
...
func (s *Service) HandleCommandResult(ctx context.Context, result *operatorv1.CommandResult) error {
	...
	eventPayload, err := protojson.MarshalOptions{UseProtoNames: true}.Marshal(result)
	if err != nil {
		return connect.NewError(connect.CodeInternal, fmt.Errorf("marshal command result event: %w", err))
	}
