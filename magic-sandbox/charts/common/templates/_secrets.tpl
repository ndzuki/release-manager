...
{{/*
Get all data from an existing secret as a dictionary for batch operations.

Usage:
{{ $secretData := include "common.secrets.getData" (dict "secret" "secret-name" "context" $) | fromYaml }}
{{ get $secretData "MY_KEY" | default "default-value" }}

Params:
  - secret - String - Required - Name of the 'Secret' resource to retrieve data from.
  - context - Context - Required - Parent context.

Returns:
  - A YAML string containing all secret data, or empty dict if secret doesn't exist
  - Must be used with `| fromYaml` to convert back to dictionary
*/}}
{{- define "common.secrets.getData" -}}
{{- $secret := (lookup "v1" "Secret" (include "common.names.namespace" .context) .secret) }}
...
{{/*
智能获取 Secret 值，自动处理 base64 编码

Usage:
{{ $secretData := include "common.secrets.getData" (dict "secret" "secret-name" "context" $) | fromYaml }}
APP_ID: {{ include "common.secrets.getValue" (dict "secretData" $secretData "key" "APP_ID" "value" .Values.appId) }}

Params:
  - secretData - Dict - Required - Secret 数据字典
  - key - String - Required - Secret 中的键名
  - value - String - Optional - 用户提供的值（未编码）
  - default - String - Optional - 默认值（未编码）

Returns:
  - Base64 编码的值
*/}}
{{- define "common.secrets.getValue" -}}
{{- if .value -}}
  {{- .value | b64enc -}}
{{- else if get .secretData .key -}}
  {{- get .secretData .key -}}
{{- else if .default -}}
  {{- .default | b64enc -}}
{{- else -}}
