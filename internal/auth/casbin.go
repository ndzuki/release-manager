...
func persistRoleBinding(m model.Model, ptype, v0, v1, v2 string) error {
	_ = m.AddPolicy(ptype, ptype, []string{v0, v1, v2}) //nolint:errcheck // model mutation failure is impossible for validated policy shapes
	return nil
}

func persistPolicy(m model.Model, ptype string, rule []string) error {
	_ = m.AddPolicy(ptype, ptype, rule) //nolint:errcheck // model mutation failure is impossible for validated policy shapes
	return nil
}
