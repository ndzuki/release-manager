...
type operatorManagementStore struct{ db *sql.DB }
...
func (s *operatorManagementStore) CreateEnrollmentToken(
	...
	var mutation *store.EnrollmentTokenMutation
	err := retryBusy(ctx, func() error {
		var err error
		mutation, err = s.createEnrollmentToken(ctx, token, replacePending, auditEvent)
		return err
	})
	return mutation, err
}
...
func insertOperatorAuditEvent(ctx context.Context, tx *sql.Tx, event *store.AuditEvent) error {
	if event == nil {
