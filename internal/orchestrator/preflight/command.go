...
func BuildUpgradeCommand(
	...
	return &operatorv1.UpgradeCommand{
		...
		Bundle: &operatorv1.ReleaseBundleSnapshot{
			BundleId:     bundle.ID,
			BundleDigest: bundle.DigestAlg + ":" + bundle.DigestValue,
			ChartRef:     bundle.ChartRef,
			ChartVersion: bundle.ChartVersion,
			ChartDigest:  bundle.ChartDigest,
		},
		Chart: &operatorv1.ChartReference{
			ResolvedUri: bundle.ChartRef,
			Version:     bundle.ChartVersion,
			Digest:      bundle.ChartDigest,
		},
		EffectiveValuesJson:    effectiveValues,
		EffectiveValuesDigest:  values.Digest(effectiveValues),
...
func normalizeImageReference(image store.BundleImage) string {
	...
	return image.Ref + "@sha256:" + digest
}
