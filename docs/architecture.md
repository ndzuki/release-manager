...
- `cmd/notification-sink/main.go` 的默认 `--config` 曾指向仓库根并不存在的 `configs/notification-sink.dev.yaml`；本次已改为实际被跟踪的 `deploy/kustomize/dev/configs/notification-sink.dev.yaml`（原先仅影响裸跑该服务）。
- `api/proto/auth/v1/auth.proto` 声明了 `ExternalIdentityService`，`cmd/auth` 仅在只读 procedure 白名单中引用其两个 RPC，未见对应 handler 挂载。
- Makefile 中 3 处 `./cmd/release-manager/` 引用与 `configs/manager.dev.yaml` 曾长期悬空（该命令在仓库中并不存在）；本次已把 `dev-stage-tenancy`、`dev-stage-config` 指向 release-orchestrator，把 `dev-stage-full` 改为导航目标，并删除孤儿配置。
