...
| `operator.http` | **协议已失效 + 端口已失效 + 契约不符**（3 条） | 打 8447（`:12,26,34`）；`OperatorService/Enroll` 的现行挂载点在 orchestrator agent gateway（`cmd/orchestrator/main.go:140-209`），dev kustomize 为 **mTLS** NodePort 30084（`deploy/kustomize/dev/configs/orchestrator.dev.yaml:22-24`）；本地 `configs/orchestrator.dev.yaml:30` 甚至 `gateway.enabled: false` |
| `audit.http` | **端口/挂载点失效 + 缺鉴权**（`EmitAudit`）；`### Health` **现状可用** | `EmitAudit` 是集合中唯一 Connect 风格路径（`:8`），但打 8083/orchestrator——AuditService 只挂在 `cmd/api`（`cmd/api/main.go:49-57`，dev 8087，`configs/api.dev.yaml:1`），且全方法要求 `Authorization: Bearer <jwt>`（`internal/audit/interceptor.go:23-44`，该请求未带）。`GET http://localhost:8083/health`（`:42-43`）在 orchestrator 运行时成立（`internal/app/app.go:139-143`） |
