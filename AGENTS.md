...
make check-docs    # 文档事实门禁（文档写出的目标/路径/链接/行号引用必须真实）
make lint-proto    # buf lint（COMMENT_* 未开启，通过不代表注释完整）
...
- **不得停止或删除常驻用户服务**（例如桌面/IDE 进程）来换取测试资源；需要端口/内存时改用自己的隔离环境。
  ⚠️ 2026-09-23：原列举的 `kb-reranker`、`ollama-sycl` **已停用**（KB 面整体归档）；不要再探测或依赖它们。
