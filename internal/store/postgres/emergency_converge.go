...
func (s *emergencyIntentStore) ConvergeEmergencyResult(ctx context.Context, command store.EmergencyConvergeCommand) (*store.EmergencyConvergeResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("begin converge emergency result: %w", err)
	}
	defer tx.Rollback() //nolint:errcheck // no-op after commit

	// Serialize concurrent writers on the same operation (matching
	// ResolveEmergencyEffect's FOR UPDATE locking convention).
	if err := lockEmergencyOperationTx(ctx, tx, command.OperationID); err != nil {
		return nil, err
	}
	...
	if current.Status.IsTerminal() {
		if intent.EffectStatus == command.EffectStatus {
			return &store.EmergencyConvergeResult{Operation: current, Intent: intent}, nil
		}
		...
		if err := tx.Commit(); err != nil {
			return nil, fmt.Errorf("commit converge emergency result: %w", err)
		}
		return &store.EmergencyConvergeResult{Operation: updatedOp, Intent: updatedIntent, Resolved: true}, nil
	}
	...
	if step.Status != store.StatusRunning {
		return nil, store.ErrInvalidState
	}
	...
	return &store.EmergencyConvergeResult{Operation: finished, Intent: updatedIntent, Resolved: true}, nil
}
...
// lockEmergencyOperationTx takes a FOR UPDATE row lock on the operation and
// its emergency intent so the subsequent read-modify-write sequence is
// serialized against concurrent ACK/finish/cancel writers.
func lockEmergencyOperationTx(ctx context.Context, tx *Tx, operationID string) error {
	var id string
	if err := tx.QueryRowContext(ctx, `SELECT id FROM operations WHERE id = ? FOR UPDATE`, operationID).Scan(&id); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return store.ErrNotFound
		}
		return fmt.Errorf("lock emergency operation: %w", err)
	}
...
func emergencyHopTx(
	...
	if err := insertOperationEvent(ctx, tx, &store.OperationStateChangedEvent{
		ID: uuid.New().String(), OperationID: updated.ID, OperationType: updated.OperationType,
...
func abandonConvergenceTaskTx(ctx context.Context, tx *Tx, operationID string) error {
...
func resolveUnknownEffectTx(
	...
	result, err := tx.ExecContext(ctx, `
		...
		WHERE id = ? AND state_version = ?
	`, now, current.ID, command.ExpectedStateVersion)
	if err != nil {
	...
	if err := writeEmergencyIntentEffect(ctx, tx, current.ID, command.EffectStatus, command.BeforeSnapshot, command.AfterSnapshot, now); err != nil {
		return nil, nil, err
	}
...
func writeEmergencyIntentEffect(ctx context.Context, execer operationExecer, operationID string, effectStatus store.EmergencyEffectStatus, beforeSnapshot, afterSnapshot json.RawMessage, now time.Time) error {
...
func (s *emergencyIntentStore) GetByID(ctx context.Context, id string) (*store.EmergencyIntent, error) {
	return scanEmergencyIntent(s.gorm.QueryRowContext(ctx, emergencyIntentSelect+` WHERE emergency_intents.id = ?`, id))
}
...
func (s *emergencyIntentStore) ListStuckLocks(ctx context.Context, filter store.StuckLockFilter) ([]*store.StuckLock, error) {
	...
	for rows.Next() {
		...
		pairs = append(pairs, pair{id: id, at: terminalAt.UTC()})
	}
...
func (s *emergencyIntentStore) ReleaseLock(ctx context.Context, command store.ReleaseLockCommand) (*store.ReleaseLockResult, error) {
	...
	tx, err := s.gorm.BeginTx(ctx, nil)
	if err != nil {
		return nil, fmt.Errorf("begin release emergency lock: %w", err)
	}
	defer tx.Rollback() //nolint:errcheck // no-op after commit
	...
	if err := lockOperationRowTx(ctx, tx, intent.OperationID); err != nil {
		return nil, err
	}
	...
	if intent.EffectStatus != store.EmergencyEffectUnknown {
		return nil, store.ErrLockNotStuck
	}
	...
	if taskErr == nil && task.Status == "pending_promotion" {
		return nil, store.ErrConvergencePending
	}
	...
	if command.Mode == store.EmergencyReleaseNotAppliedProven {
		opResult, err := tx.ExecContext(ctx, `
			UPDATE operations SET state_version = state_version + 1, updated_at = ?
			WHERE id = ? AND state_version = ?
		`, now, current.ID, current.StateVersion)
		if err != nil {
			return nil, fmt.Errorf("release emergency lock operation effect: %w", err)
		}
		opRows, err := opResult.RowsAffected()
		if err != nil {
			return nil, fmt.Errorf("release emergency lock operation rows: %w", err)
		}
		if opRows == 0 {
			return nil, store.ErrOptimisticLock
		}
		...
		if err != nil {
			return nil, fmt.Errorf("encode release emergency lock timeline: %w", err)
		}
		if _, err := appendTimelineEntry(ctx, tx, &store.OperationTimelineEntry{
	...
	effectResult, err := tx.ExecContext(ctx, `
		...
		WHERE id = ?
	`, string(releaseEffectStatus(command.Mode)), now, now, command.IntentID)
	if err != nil {
		return nil, fmt.Errorf("release emergency lock: %w", err)
	}
	effectRows, err := effectResult.RowsAffected()
	if err != nil {
		return nil, fmt.Errorf("release emergency lock rows: %w", err)
	}
	if effectRows == 0 {
		return nil, store.ErrLockNotStuck
	}
	if err := tx.Commit(); err != nil {
		return nil, fmt.Errorf("commit release emergency lock: %w", err)
	}
	updatedIntent := *intent
	...
	return &store.ReleaseLockResult{Intent: &updatedIntent, Operation: &updatedOp}, nil
}
...
// lockOperationRowTx takes a FOR UPDATE row lock on the operation.
func lockOperationRowTx(ctx context.Context, tx *Tx, operationID string) error {
	var id string
	if err := tx.QueryRowContext(ctx, `SELECT id FROM operations WHERE id = ? FOR UPDATE`, operationID).Scan(&id); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return store.ErrNotFound
