...
| 8bis | 制品准入（漏洞）**已接线但默认影子**（TASK-105）：`vulnerability_admission.mode` 三态（`off`/`shadow`/`enforce`），默认 `shadow` 放行并记录「本应拒绝」；`enforce` 对 reject 与评估不可用分别以 `vulnerability_policy_failed`/`vulnerability_policy_unavailable` 拒绝。**注意：仓库只有 `vulnerability.Scanner` 接口、无生产 scanner 实现，且 `SetVulnerabilityEvaluator` 尚无生产调用者**，故现状下结论恒为 `vulnerability_policy_unavailable`（接入 evaluator 但缺扫描结果时则是 `vulnerability_policy_failed`），两种情况下 `enforce` 都会阻断发布；runbook §3bis 给出影子→enforce 流程、判读要点与逃生门 | 部分实现（缺 scanner） | `internal/orchestrator/admission.go`、`internal/config/config.go`、`docs/runbook.md` §3bis |
