...
func TestCertificateHotReload(t *testing.T) {
	...
	resp, err := clientA.HealthCheck(ctx, &releasev1.HealthCheckRequest{})
	require.NoError(t, err, "health check with cert-A")
	...
	resp, err = clientB.HealthCheck(ctx, &releasev1.HealthCheckRequest{})
	require.NoError(t, err, "health check with cert-B")
	assert.True(t, resp.Healthy)
	connB.Close()
