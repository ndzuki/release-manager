...
func TestValuesRevisionConvergenceArraysRoundTrip(t *testing.T) {
	...
	vr := &store.ValuesRevision{
		...
		Status:              store.ValuesStatusDraft,
		CanonicalDocument:   []byte(`{}`),
		ConvergenceTaskIds:  []string{"task-1", "task-2"},
	...
	got, err := st.Values().Get(ctx, vr.ID)
	require.NoError(t, err)
	assert.Equal(t, []string{"task-1", "task-2"}, got.ConvergenceTaskIds)
	assert.Equal(t, []string{"api.image.digest", "web/*"}, got.LockedPaths)

	// Empty bindings read back as nil (never as a spurious slice).
	plain := &store.ValuesRevision{
		ID: uuid.NewString(), ReleaseDefinitionID: def.ID, Version: 2,
		Status: store.ValuesStatusDraft, CanonicalDocument: []byte(`{}`),
		ParentRevisionID: vr.ID,
	}
	require.NoError(t, st.Values().Create(ctx, plain))
	plainGot, err := st.Values().Get(ctx, plain.ID)
	require.NoError(t, err)
	assert.Nil(t, plainGot.ConvergenceTaskIds)
	assert.Nil(t, plainGot.LockedPaths)
}
