...
UPDATE authorization_source_version SET version = ? WHERE id = TRUE AND version = ?`, nextSource, current.SourceVersion)
	if err != nil {
	...
	result, err = tx.ExecContext(ctx, `UPDATE policy_version SET version = ? WHERE id = TRUE AND version = ?`, nextPolicy, current.PolicyVersion)
	if err != nil {
...
WHERE excluded.source_version > authorization_checkpoints.source_version
   OR (excluded.source_version = authorization_checkpoints.source_version
       AND excluded.policy_version >= authorization_checkpoints.policy_version)`,
		...
		checkpoint.PolicyVersion, checkpoint.Fresh, checkpoint.UpdatedAt.UTC(),
	)
	if err != nil {
...
FROM capability_grants ORDER BY organization_id, subject, action`)
	...
	for grantRows.Next() {
		...
		if err := grantRows.Scan(&grant.OrganizationID, &grant.Subject, &grant.Action, &grant.GrantedBy,
			&grant.Revoked, &grant.CreatedAt, &grant.UpdatedAt); err != nil {
...
func loadAuthorizationVersions(ctx context.Context, queryer interface {
	...
	if err := queryer.QueryRowContext(ctx, `SELECT version FROM authorization_source_version WHERE id = TRUE`).Scan(&snapshot.SourceVersion); err != nil {
		return nil, fmt.Errorf("load authorization source version: %w", err)
	}
	if err := queryer.QueryRowContext(ctx, `SELECT version FROM policy_version WHERE id = TRUE`).Scan(&snapshot.PolicyVersion); err != nil {
...
VALUES (?, ?, ?, ?, ?, ?, ?)`, grant.OrganizationID, grant.Subject, grant.Action, grant.GrantedBy,
		grant.Revoked, grant.CreatedAt.UTC(), grant.UpdatedAt.UTC())
	if err != nil {
...
func scanAuthorizationCheckpoint(row interface{ Scan(...any) error }) (*store.AuthorizationCheckpoint, error) {
	...
	if err := row.Scan(&checkpoint.OrganizationID, &checkpoint.CustomerID, &checkpoint.SourceVersion,
		&checkpoint.PolicyVersion, &checkpoint.Fresh, &checkpoint.UpdatedAt); err != nil {
